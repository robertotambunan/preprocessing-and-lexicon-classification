...
cursor = connection.cursor()
connectionDoc = MySQLdb.connect (host = "127.0.0.1", user = "root", passwd = "", db = "naivebayes")
cursorDoc = connectionDoc.cursor()
...
for row in data:
	...
	#	print "Hasil Stemming"
		...
		print "Tweet:\n" + sentence
		print "Preprocessing:\n"+stemmingResult
		cursorDoc.execute("SELECT SUM(jumlah) FROM doc_positif")
		sumPositif = cursorDoc.fetchone()[0]
		cursorDoc.execute("SELECT SUM(jumlah) FROM doc_negatif")
		sumNegatif = cursorDoc.fetchone()[0]
		cursorDoc.execute("SELECT SUM(jumlah) FROM doc_netral")
		sumNetral = cursorDoc.fetchone()[0]				
		cursorDoc.execute("SELECT count(*) FROM doc_positif")
		cntPositif = cursorDoc.fetchone()[0]
		cursorDoc.execute("SELECT count(*) FROM doc_negatif")
		cntNegatif = cursorDoc.fetchone()[0]
		cursorDoc.execute("SELECT count(*) FROM doc_netral")
		cntNetral = cursorDoc.fetchone()[0]
		for splitter in splitWord:
			cursorDoc.execute("SELECT jumlah FROM doc_positif WHERE  kata='%s' "% (splitter))
			crsKataPos = cursorDoc.fetchone()
			cursorDoc.execute("SELECT jumlah FROM doc_negatif WHERE  kata='%s' "% (splitter))
			crsKataNeg = cursorDoc.fetchone()
			cursorDoc.execute("SELECT jumlah FROM doc_netral WHERE  kata='%s' "% (splitter))
			crsKataNet = cursorDoc.fetchone()
			if crsKataPos:
